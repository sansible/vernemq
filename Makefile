...
watch: test_deps
	while sleep 1; do \
		find defaults/ handlers/ meta/ tasks/ tests/test.yml \
...
## Start and (re)provision Vagrant test box
vagrant:
...
lint: .venv_$(ANSIBLE_INSTALL_VERSION)
	find defaults/ handlers/ meta/ tasks/ -name "*.yml" | xargs -I{} ansible-lint {}
